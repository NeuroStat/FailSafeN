    ...
    for (seed in seeds) {
        ...
        ###########################################
                ...
                file<-file[!is.na(file[,1]),1]
                    ...
                    # is the target cluster present? (3+5stud-18_20_p001_clust)
                    setwd(image)
                    imagename<-system("ls FSNmat_vFWE05*",intern=TRUE)
...
###########################################
    ...
    write.table(cbind(seed,FSNseed,percrs),paste("FSN_FWER_",threshFWER,"_",varppn,"_1.txt", sep=""),quote=FALSE,row.names=FALSE,col.names=FALSE,append=TRUE)
}}
