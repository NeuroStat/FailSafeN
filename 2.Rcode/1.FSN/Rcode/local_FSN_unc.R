    ...
    for (seed in seeds) {
        ...
        ###########################################
                ...
                file<-file[!is.na(file[,1]),1]
                    ...
                    setwd(image)
                    imagename<-system("ls FSNmat_*",intern=TRUE)
...
###########################################
    ...
    write.table(cbind(seed,FSNseed,percrs),paste("FSN_unc_",threshunc,"_",varppn,"_1.txt", sep=""),quote=FALSE,row.names=FALSE,col.names=FALSE,append=TRUE)
}}
